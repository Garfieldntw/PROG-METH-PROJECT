...
public class GameLogic {
	...
	public static void DropWeapon(int xPosition, int yPosition) {
		...
		if(randomIndex == 2) GameController.getGamePane().getDropCanvas().Drop(new Shovel(2),
		xPosition, yPosition,
		GameController.getWeaponCanvas().getGraphicsContext2D());
		else if(randomIndex == 3) GameController.getGamePane().getDropCanvas().Drop(new Rock(2),
		xPosition, yPosition,
		GameController.getWeaponCanvas().getGraphicsContext2D());
		// GameController.getGamePane().getDropCanvas().Drop(new Shovel(2), xPosition, yPosition,
				// GameController.getWeaponCanvas().getGraphicsContext2D());
	...
	// return the pane that topright(TR) pixel of the player is in (Used for collision logic)
